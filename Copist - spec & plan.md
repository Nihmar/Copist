...
*   **Editor conveniences:** spellcheck (Android/iOS: the system/IME proofreader; Linux/Windows: no engine-provided service, backlog — `ANALYSIS.md` § "Platform parity", T-PP-09), word count, heading outline + folding, and the formatting toolbar (bold, italic, strikethrough, superscript, underline, link, code, image, list, quote — mockup-confirmed, evenly spaced). LaTeX autocomplete = backlog.
