...
Spec: `Copist - spec & plan.md`. Pending points (replaces the former
`plan/` folder): `ANALYSIS.md`.
...
- Android storage: the library is read/written with plain `dart:io` (walk, watch, open by path), so it needs `MANAGE_EXTERNAL_STORAGE` ("All files access"), gated by `core/storage_access.dart`. A SAF tree grant is **not** a substitute — it only opens the DocumentsProvider (`content://`), never the filesystem; two rounds were lost to that. The picker only *chooses* the root. Details: `ANALYSIS.md` (former `plan/android.md`, all fixed).
...
- Never walk the disk or read file content on the UI isolate: on Android every `listSync`/`statSync` is a FUSE round trip and hashing reads whole files, which is how the app earned an ANR (former `plan/android.md` issue 3, fixed — see `ANALYSIS.md`). Use `Isolate.run`; the drift writes stay on the main isolate.
