...
lib/
  ...
  src/
    ...
    editor/
      source_editor.dart    # custom lightweight editor (single buffer)
      highlighting.dart     # MD + math-span tokenizer → styled display
    preview/
      markdown_view.dart    # flutter_markdown preview
...
`frontmatter_fields`). Search = FTS5 (title + body) plus tag lookup.
...
- **Editor:** custom lightweight source editor over a single text buffer per
  note. Highlighting (Markdown tokens, math spans `$…$`/`$$…$$`) is a display
  layer over plain text — the edit model never becomes rich text.
- **Preview:** `flutter_markdown` render, parsed once per change (debounced),
  lazy block layout. Extras: tables, task lists, footnotes, strikethrough,
  code blocks via `flutter_highlight`.
- **Math:** math spans extracted from source → `katex_flutter` (pure-Dart
...
| CI                 | `flutter analyze` + `flutter test` (M0); build artifacts added in M7 |
...
- **1M gate (M6):** fixture library + timing assertions in CI.
