#!/usr/bin/env bash
# Build a Copist AppImage from a Flutter Linux release bundle.
#
# Usage:
#   packaging/appimage/make-appimage.sh <bundle-dir> <icon-png> <version> <output>
#
#   <bundle-dir>  e.g. build/linux/x64/release/bundle (contains copist,
#                 data/, lib/ and dev.copist.copist.desktop)
#   <icon-png>    e.g. assets/branding/feather.png
#   <version>     e.g. 1.2.3 (used for update info only)
#   <output>      e.g. dist/copist-1.2.3-linux-x64.AppImage
#
...
for f in "$BUNDLE/copist" "$BUNDLE/dev.copist.copist.desktop" "$ICON"; do
  if [ ! -e "$f" ]; then
...
sed -e 's|^Icon=.*|Icon=copist|' \
  "$BUNDLE/dev.copist.copist.desktop" > "$APPDIR/copist.desktop"
cp "$ICON" "$APPDIR/copist.png"
cp "$ICON" "$APPDIR/usr/share/icons/hicolor/256x256/apps/copist.png"
