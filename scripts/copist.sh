#!/usr/bin/env bash
# Copist dev helper: terse output, full logs in /tmp/copist/copist-<cmd>.log.
...
linux_build() {
  ...
  tail -n 3 "$log"
  if [ $status -eq 0 ]; then
    echo "artifact: build/linux/x64/release/bundle/copist"
  fi
  return $status
}
...
usage() {
  cat <<'EOF'
usage: ./scripts/copist.sh <analyze|test|check|apk|linux>
...
Full logs: /tmp/copist/copist-<cmd>.log
EOF
}
