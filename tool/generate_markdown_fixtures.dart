...
final class _Gen {
  ...
  String _inlineSpan() {
    ...
    if (span < 70) {
      return '[${_word()} link](https://example.com/${_randInt(1, 999)})';
    }
  ...
  void _heading() {
    ...
    final hashes = '#' * level;
    _emitLine(
      '$hashes Section $_headingCounter ${_word().capitalizeFirst} '
  ...
  void _bulletList() {
    ...
    for (var i = 0; i < n; i++) {
      final check = task ? (rand.nextBool() ? '[x] ' : '[ ] ') : '';
  ...
  void _imageParagraph() {
    _emitLine(
      '![Figure ${_randInt(0, 999)}](assets/img-${_randInt(0, 999)}.png)',
    );
  }
